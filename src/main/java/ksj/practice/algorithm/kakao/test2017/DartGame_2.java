 ...
 */
public class DartGame_2 {

    public static void main(String[] args) {
        List<String> example = new ArrayList<>();
        example.add("1S2D*3T");     // 37
        example.add("1D2S#10S");    // 9
        example.add("1D2S0T");     // 3
        example.add("1S*2T*3S");     // 23
        example.add("1D#2S*3S");     // 5
        example.add("1T2D3D#");     // -4
        example.add("1D2S3T*");     // 59

        for(String ex : example) {
            System.out.println(ex);
            StringToIntDartScore(ex);
        }

    }

    static void StringToIntDartScore(String dart) {
        ...
        while(matcher.find()) {
            ...
            int score = (int)Math.pow(number,bonus) * starBonus;
            scoreList.add(score);
        }
        int scoreSum = 0;
